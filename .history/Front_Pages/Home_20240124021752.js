...
const Home = ({navigation}) =>{
  ...
  {camera: "Camera", onPress: {cameratake}},
  {gallery: "Gallery", onPress: {galleryupload}}])
  ...
  const galleryupload = async () => {
    ...
    let gallerylanuch = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: ImagePicker.MediaTypeOptions.Images,
      aspect: [1, 1],
      quality: 1
    })

    if (!gallerylanuch.canceled){
      setAvatarSource(gallerylanuch.assets)
    }
  }
